...
function renderZoneSummary() {
  const list = current().filter(f => Number.isFinite(f.lat) && Number.isFinite(f.lng));
  const isDbscan = mode === 'dbscan';
...
function renderMap() {
  const mappedList = current().filter(f => Number.isFinite(f.lat) && Number.isFinite(f.lng));
  renderZoneSummary();
  const list = mappedList;
  const normal = list.filter(f => !f.outlier).length, out = list.length - normal;
...
function showDialog(html) {
  ...
  $$('#dialog-content .detail-box').forEach(box => {
    const heading = box.querySelector('strong')?.textContent.trim() || '';
    const summary = box.querySelector('summary')?.textContent.trim() || '';
    if (heading === 'Apa arti kemiripan data?' || heading === 'Lokasi fasilitas' || summary === 'Penjelasan teknis untuk penelitian') box.remove();
  });
  $$('#dialog-content p').forEach(paragraph => {
    if (paragraph.textContent.trim() === 'Seluruh catatan pemeriksaan pada periode ini disertakan. Nama dan identitas pasien tidak ditampilkan.') paragraph.remove();
  });
...
function methodology() {
  ...
  if (!dialog) {
    ...
    dialog.innerHTML = `<button class="dialog-close" id="intro-close" aria-label="Tutup pengantar">×</button>
      ...
      <section class="intro-visual-layout">
        <div class="intro-map-preview intro-real-map" aria-label="Peta nyata Kabupaten Cianjur dengan lokasi fasyankes">
          <div id="intro-map"></div>
          <div class="preview-callout"><b>Peta fasyankes</b><strong>Cianjur</strong><small>Lokasi berdasarkan koordinat referensi</small></div>
          <div class="preview-legend"><span><i></i> Rendah</span><span><i></i> Sedang</span><span><i></i> Tinggi</span></div>
        </div>
  ...
  if (!dialog.open) { dialog.showModal(); dialog.scrollTop = 0; $('#intro-title').focus({ preventScroll: true }); window.setTimeout(ensureIntroMap, 80) }
}
