...
let page='beranda',mode='kmeans',riskFilter='Semua',chartType='distribution',legendFilter=null,tablePage=1;
...
function renderTable(){const q=$('#search').value.toLowerCase(),list=current().filter(f=>(riskFilter==='Semua'||(riskFilter==='Outlier'?f.outlier:riskFilter==='Normal'?!f.outlier:f.risk===riskFilter))&&`${f.name} ${escapeHTML(f.area)}`.toLowerCase().includes(q)),pageSize=10,totalPages=Math.max(1,Math.ceil(list.length/pageSize));tablePage=Math.min(tablePage,totalPages);const visible=list.slice((tablePage-1)*pageSize,tablePage*pageSize);$('#result-count').textContent=`${list.length} FASYANKES`;$$('[data-risk]').forEach(b=>{b.classList.toggle('active',b.dataset.risk===riskFilter);b.setAttribute('aria-pressed',b.dataset.risk===riskFilter)});$('#facility-table').innerHTML=visible.map(f=>`<tr data-id="${f.id}" tabindex="0"><td><strong>${escapeHTML(f.name)}</strong><small>${escapeHTML(f.area)}</small></td><td>${f.samples}</td><td>${f.mean.toFixed(2).replace('.',',')} µU/mL</td><td>${f.tsh.toFixed(1).replace('.',',')} µU/mL</td><td>${badge(f.risk)}</td><td style="color:${f.outlier?'#ed6c70':'#8ba0b2'}">${f.outlier?'↗ Outlier':'Normal'}</td><td><button class="text-button" aria-label="Detail ${escapeHTML(f.name)}">Detail ↗</button></td></tr>`).join('')||'<tr><td colspan="7">Tidak ada fasilitas yang sesuai. Coba kata kunci atau filter lain.</td></tr>';$('#table-pagination').innerHTML=`<button class="page-arrow" data-page="prev" aria-label="Halaman sebelumnya" ${tablePage===1?'disabled':''}>‹</button>${Array.from({length:totalPages},(_,i)=>`<button class="page-number ${tablePage===i+1?'active':''}" data-page="${i+1}" aria-label="Halaman ${i+1}" aria-current="${tablePage===i+1?'page':'false'}">${i+1}</button>`).join('')}<button class="page-arrow" data-page="next" aria-label="Halaman berikutnya" ${tablePage===totalPages?'disabled':''}>›</button><span class="page-status">Halaman ${tablePage} dari ${totalPages}</span>`;$$('[data-page]').forEach(b=>b.onclick=()=>{if(b.disabled)return;tablePage=b.dataset.page==='prev'?tablePage-1:b.dataset.page==='next'?tablePage+1:Number(b.dataset.page);renderTable()});bindDetails()}
