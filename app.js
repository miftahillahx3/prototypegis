...
function paginationPages(total,current){if(total<=5)return Array.from({length:total},(_,i)=>i+1);const pages=[1];if(current>3)pages.push('dots-left');if(current>2&&current<total-1)pages.push(current);if(current<total-2)pages.push('dots-right');pages.push(total);return pages}
